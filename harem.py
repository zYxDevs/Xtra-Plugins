...
async def harem_catcher(client, message):
    ...
    gues = guess.replace("cg artwork", "Stella")
    gue = gues.replace("Umineko When They Cry", "Lambdadelta")
    guee = gue.replace("Hanasaku Iroha", "Ohana Matsumae")
    guea = guee.replace("fiction", "5pb")
    gueb = guea.replace("noire choujigen game neptune the animation", "uni")
    guec = gueb.replace("klaxosaur", "Kyouryuu")
    gued = guec.replace("Sewayaki Kitsune no Senko-san", "Sora")
    guef = gued.replace("cleavage", "quinella")
    gueg = guef.replace("saya akdepsksal", "ange")
    gueh = gueg.replace("for teen", "bridged")
    guei = gueh.replace("higehiro characters name list", "yuzuha")
    guej = guei.replace("5pbal character", "caren")
    guek = guej.replace("fictional character", "caren")
    guel = guek.replace("pfp demon slayer aesthetic", "mitsuri")
    guem = guel.replace("kimi da yo kimi nanda yo", "kaori")
    guesa = guem.replace("yuru yuri", "ayano")
