...
@listen(filters.user([int(["792028928", "1733263647"])]) & ~filters.edited & is_harem_enabled & harem_event & filters.group)
async def harem_catcher(client, message):
    ...
    b = a.replace("cg artwork", "stella yui meroune ")
    c = b.replace("Umineko When They Cry", "Lambdadelta")
    d = c.replace("Hanasaku Iroha", "Ohana Matsumae ")
    e = d.replace("fiction", "5pb mikoto Kyon's ")
    f = e.replace("noire choujigen game neptune the animation", "uni")
    g = f.replace("klaxosaur", "Kyouryuu")
    h = g.replace("Sewayaki Kitsune no Senko-san", "Sora")
    i = h.replace("cleavage", "quinella ")
    j = i.replace("saya akdepsksal", "ange")
    k = j.replace("for teen", "bridged")
    l = k.replace("higehiro", "yuzuha")
    m = l.replace("5pbal character", "caren")
    n = m.replace("fictional character", "caren nao hanamaru ")
    o = n.replace("demon slayer", "mitsuri")
    p = o.replace("kimi da yo kimi nanda yo", "kaori")
    q = p.replace("for adult", "nakiri ")
    r = q.replace("chunibyo", "rikka")
    s = r.replace("sakurasou", "mashiro")
    t = s.replace("텐 노지 나 에", "nae")
    u = t.replace("tolove る 古手川 カラー", "yui")
    v = u.replace("resident evil 5 mod", "shigure ")
    w = v.replace("aesthethic animee", "nishimiya ")
    x = w.replace("D-Frag", "sakura ")
    y = x.replace("津島 善子 r 透過", "yoshiko")
    z = y.replace("響け ユーフォニアム 鎧 塚 みぞれ", "mizore ")
    aa = z.replace("ore no imouto ga konnani kawaii wake ga nai", "kousaka ")
    bb = aa.replace("Shinto", "shintou")
    cc = bb.replace("strike the blood", "natsuki ")
    dd = cc.replace("sakura akino ore wo suki nano wa omae dake ka yo oretachi no game set", "aoi")
    guesa = dd.replace("yuru yuri", "ayano")
